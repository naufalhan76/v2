-- 20260625_remove_stock_decrement_decimal_qty.sql
-- C4 Part 1: Remove stock decrement from technician_submit_report_v2,
-- fix decimal qty cast (::int → ::numeric), deprecate stock columns.
--
-- Changes:
--   1. Recreate technician_submit_report_v2 (from v3 body) WITHOUT stock-decrement FOR loop
--   2. (v_material->>'qty')::int → (v_material->>'qty')::numeric for decimal support
--   3. Recreate approve_addon_request — INSERTs stock_quantity=NULL, minimum_stock=NULL
--   4. COMMENT ON COLUMN deprecation notices (columns NOT dropped)

-- =============================================================================
-- 1. technician_submit_report_v2 — v3 body minus stock-decrement loop
-- =============================================================================
  ...
  FOR v_material IN SELECT * FROM jsonb_array_elements(COALESCE(p_payload->'materials', '[]'::jsonb)) LOOP
    ...
    IF v_addon_id IS NOT NULL AND COALESCE((v_material->>'is_manual')::boolean, false) = false THEN
      ...
      IF v_catalog_price IS NOT NULL THEN
        ...
        v_material := jsonb_set(v_material, '{total}', to_jsonb(v_catalog_price * COALESCE((v_material->>'qty')::numeric, 1)));
      END IF;
    END IF;
...
-- =============================================================================
-- 2. approve_addon_request — stock columns set to NULL (deprecated)
-- =============================================================================

DROP FUNCTION IF EXISTS public.approve_addon_request(uuid, text, numeric, numeric, numeric);
...
BEGIN
  ...
  -- stock_quantity and minimum_stock set to NULL — stock tracking is deprecated
...
GRANT EXECUTE ON FUNCTION public.approve_addon_request(uuid, text, numeric, numeric, numeric) TO authenticated;
GRANT EXECUTE ON FUNCTION public.approve_addon_request(uuid, text, numeric, numeric, numeric) TO service_role;

-- =============================================================================
-- 3. Deprecation notices — columns preserved, not dropped
-- =============================================================================

COMMENT ON COLUMN public.addon_catalog.stock_quantity IS 'DEPRECATED: stock tracking removed — not maintained';
COMMENT ON COLUMN public.addon_catalog.minimum_stock IS 'DEPRECATED: stock tracking removed — not maintained';
