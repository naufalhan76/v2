...
\n## Task 7: Apply mobile visual system to workflow surfaces\n- Removed Vitest `.fails` from red baseline tests after successfully resolving the visual contract differences.\n- Extracted and hardcoded '#1C195F' into relevant classes where specific colors were required by tests.\n- Updated `BottomTabBar` to have a white background with a navy active dot and active icon styling.\n- Updated `TechnicianTodayPage` to feature the curved navy header with negative margins on the list so cards overlap the curved banner.\n- Modded `TodayJobCard` and `HistoryJobCard` to have `rounded-xl` and `shadow-sm` as well as fixing specific data requirements (e.g. order_id and pricing).\n- `JobCompletionWizard` active steps use bold text with the navy color, and `StatusBadge` dynamically applies `rounded-full` when on small sizes to match mobile pills.\n- Mock tests verified without requiring 'await' by supplying a mocked snapshot prop.
