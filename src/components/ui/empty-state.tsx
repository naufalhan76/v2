...
export function EmptyState({ icon: Icon, title, description, action, className }: EmptyStateProps) {
  return (
    <div className={cn('flex flex-col items-center justify-center py-10 sm:py-12 px-4 text-center', className)}>
      <div className="flex h-14 w-14 items-center justify-center rounded-full bg-surface-muted mb-4 empty-state-item">
      ...
      {description && (
        <p className="text-sm text-muted-foreground max-w-[280px] sm:max-w-sm mb-4 empty-state-item">
          {description}
        </p>
      )}
      {action && (
