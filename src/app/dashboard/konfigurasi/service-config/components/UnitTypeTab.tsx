...
export function UnitTypeTab() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let res;
    if (editingItem) {
      res = await updateUnitType(editingItem.unit_type_id, input)
    } else {
      res = await createUnitType(input)
    }
  ...
  const handleDelete = async () => {
    ...
    setIsLoading(true)
    const res = await deleteUnitType(deletingItem.unit_type_id)
    if (res.success) {
    ...
    <Card className="rounded-xl border border-border/50 shadow-sm">
      ...
      <CardContent>
        ...
        <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
          <AlertDialogContent>
            <AlertDialogHeader><AlertDialogTitle>Hapus Unit Type?</AlertDialogTitle></AlertDialogHeader>
            <AlertDialogFooter>
